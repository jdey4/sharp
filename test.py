...
total_layers, short_term_memory = 2, 5
...
for X, y in loader:
    ...
    for l in range(1, total_layers):
        ...
        if (total % stride == 0):
            ...
            with torch.no_grad():
                ...
                cntxt[l-1] = pred_blocks[l](h_states[l], cntxt[l] if l<total_layers-1 else None)
    ...
    if total % sleep_interval_wake == 0:
        ...
        for l in range(1, total_layers):
            ...
            sleep_train_layer(l, sleep_steps_per_L[l], short_term_memory, mem_blocks, mem_opts, mem_criteria, pred_blocks, pred_opts, pred_criteria, sigma=0.0, ema_alpha=ema_alpha)
